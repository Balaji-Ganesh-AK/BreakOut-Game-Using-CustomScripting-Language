...
namespace MegaScrypt
{
    public class Processor : MegaScryptBaseVisitor<object>
    {
        ...
        protected object BinaryOperationHandler(object objectA, object objectB, ITerminalNode node)
        {
            ...
            else if (objectA is float && objectB is float)
            {
                return BinaryFloatOp(objectA, objectB, node);
            }
            else if ((objectA is string || objectB is string) && node.Symbol.Type == MegaScryptParser.Add)
            {
                return BinaryStringSplOp(objectA, objectB, node);
            }
            else if (objectA is float || objectB is float)
            {
                return BinaryFloatOp(objectA, objectB, node);
        ...
        private object BinaryStringSplOp(object objectA, object objectB, ITerminalNode node)
        {
        ...
        protected object BinaryFloatOp(object objectA, object objectB, ITerminalNode node)
        {
            ...
            try
            {
                 a = Convert.ToSingle(objectA);
                 b = Convert.ToSingle(objectB);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public override object VisitForLoop(MegaScryptParser.ForLoopContext context)
        {
            ...
            while (true)
            {
                ...
                //case Continue
                 if (returnState == ReturnStateEnum.Cont)
                 {
                     IsReturnStateProcessed = true;
                 }
        ...
        public override object VisitWhileLoop([NotNull]MegaScryptParser.WhileLoopContext context)
        {
            while ((bool) context.expression().Accept(this))
            {
                if (returnState == ReturnStateEnum.Break)
